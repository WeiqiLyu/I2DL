...
class CrossEntropyForwardTest(UnitTest):
    def __init__(self, loss):
        # self.value = loss.forward(np.array([0.7, 0.2]), np.array([3., 4.]))
        # self.truth = np.array([])
        pass
    ...
    def define_failure_message(self):
        # return "Cross Entropy forward incorrect. Expected: " + \
        #     str(self.truth) + " Evaluated: " + str(self.value)
        pass

class CrossEntropyBackwardTest(UnitTest):
    def __init__(self, loss):
        # self.value = loss.backward(np.array([[0.5, 0.3]]), np.array([2., 4.]))
        # self.loss = loss
        # self.error = 0.0
    ...
    def define_failure_message(self):
        # return "Cross Entropy backward incorrect. Expected: < 1e-8 Evaluated: " + \
        #     str(self.error)
        pass

class CrossEntropyTest(CompositeTest):
    def define_tests(self, loss):
        return [
            CrossEntropyForwardTest(loss),
            CrossEntropyBackwardTest(loss)
        ]
